...
for row in range(len(image)):
    for column in range(len(image[row])):
        ...
        # Finding pixel average:
        pixel_average = ((rgb_pixel[0] + rgb_pixel[1] + rgb_pixel[2]) / 3)
        # Reassigning all colors to the final average:
        rgb_pixel[0] = pixel_average
        rgb_pixel[1] = pixel_average
        rgb_pixel[2] = pixel_average
